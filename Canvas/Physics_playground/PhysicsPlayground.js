// from here https://codepen.io/dissimulate/pen/fhjvk
...
Vector.prototype.sub = function(v) {
    if (v.x != null && v.y != null) { this.x -= v.x; this.y -= v.y; } 
...
Vector.prototype.distance = function(v){ var x = this.x - v.x; var y = this.y - v.y; return Math.sqrt(x * x + y * y); };
Vector.prototype.reset    = function() { this.x  = 0;  this.y = 0;   return this; };
Vector.prototype.neg      = function() { this.x *= -1; this.y *= -1; return this; };

Vector.add = function(v1, v2) {
    if (v2.x != null && v2.y != null) { return new Vector( v1.x + v2.x , v1.y + v2.y); } 
...
Point.prototype.check_walls = function(x, y, w, h) {
    ...
    if (this.pos.y >= h - 1) this.pos.x -= (this.pos.x - this.pre.x + this.acc.x);
};
...
Constraint.prototype.resolve = function() {
    ...
    //dists.normalize();
    var f = dists.mul(diff * 0.5/length );
    this.p1.move(f);
    this.p2.move(f.neg());
};
...
Constraint.prototype.draw = function(ctx, stress) {
    if(stress) {
        ...
        ctx.strokeStyle = 'rgba('+per+', 0, 0, 0.8)';
    } else ctx.strokeStyle = 'rgba(0,0,0,0.8)';
    ctx.beginPath();
