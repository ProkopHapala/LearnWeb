...
function getUniformLocations( shader, names ){
    let ulocs = {};
    for(let i=0; i<names.length; i++){ let name=names[i]; ulocs[name] = gl.getUniformLocation(shaderProgram, names ); };
    return ulocs;
}
...
//        uniformLocations: getUniformLocations( shaderProgram, ["projectionMatrix","modelViewMatrix","txAtoms","txLJ"] + [ "dfrenorm", "dfmin","z0", "Krad","Klat","Rlever","dt","cdamp","f2conv","f2max" ] ),
    };
...
function updateUniforms(){
    ...
    let inp_names = [ "dfrenorm", "dfmin", "z0", "Krad","Klat","Rlever","dz","dt","cdamp","fconv","fmax" ];
    ...
    updateFloatUniforms( locs, [ "dfrenorm", "dfmin","z0", "Krad","Klat","Rlever","dt","cdamp","f2conv","f2max" ], inps );
